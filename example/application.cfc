component {
	...
	public any function onCFCRequest(
		...
		if (isDefined("metadata.jdsAutoDeserialize")) {// deserialize WEB service arguments if Auto Deserialization enabled
			local.deserializedArguments = new lib.JSONDeserializer().deserializeMethodArguments(arguments.args, metadata.parameters);
			return invoke(componentInstance, arguments.method, deserializedArguments);
		}
